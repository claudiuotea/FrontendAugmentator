...
import { LinkCompInterface, LinkItemInterface, Links } from "./Links";
...
export const Stats: React.FunctionComponent<{}> = () => {
  ...
  const [currentComponent,setCurrentComponent] = useState('barData')
 ...
 const renderSwitch = () =>{
    switch(currentComponent){
       ...
       case 'erasePieData': return (
       <div style={{position:"relative",height:"50vh",width:"50vw"}}><Pie data={erasePieData} options={{
         responsive: true,
         maintainAspectRatio: false,
       }} type/>
       </div>
       );
       case 'flipPieData': return (
         <div style={{position:"relative",height:"50vh",width:"50vw"}}><Pie data={flipPieData} options={{
           responsive: true,
           maintainAspectRatio: false,
         }} type/>
         </div>
       ...
       case 'linksPanel': {
          ...
          var itemProps : LinkItemInterface[] = []
         for(var i = 0; i < filenames.length; i++)
          {
             let item : LinkItemInterface ={
                filename: filenames[i],
                link : links[i]
             }
             itemProps.push(item)
          }
         return <Links items={itemProps}/>
       }
    }
 }
  return (
    <>
      ...
      <FormLabel component="legend">Choose what would you like to see</FormLabel>
      <RadioGroup aria-label="components" name="components" value={currentComponent} onChange={handleChange}>
        <FormControlLabel value="barData" control={<Radio />} label="Overall" />
        <FormControlLabel value="erasePieData" control={<Radio />} label="Erase options" />
        <FormControlLabel value="flipPieData" control={<Radio />} label="Flip and rotate options" />
        {isAdmin?null:<FormControlLabel value="linksPanel" control={<Radio />} label="My augmentations" />}
      </RadioGroup>
    </FormControl>
    {renderSwitch()}
    </>
